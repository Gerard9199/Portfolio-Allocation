...
with st.sidebar:
    ...
    with st.expander('Stocks'):
        for i in range(int(N)):
            stockList.append(st.text_input(f'Ticker {i+1}', key=f'i{key}'))
            key+=1
    ...
    if Q1_value == 'Yes':
        with st.expander('Black-Litterman Views'):
            ...
            for i in range(views_nbr):
                views[i]={}
                views[i]['Cr'] = st.text_input(f'Ticker which will go up', key=f'i{key}')
                key+=1
                views[i]['Dr'] = st.text_input(f'Ticker which will go down', key=f'i{key}')
                key+=1
    ...
    _Capital = Capital
    DELTA = st.number_input('Re-allocation time', min_value=1, step=1)
